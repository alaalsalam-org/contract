...
class JournalEntry(AccountsController):
    def _init_(self, *args, **kwargs):
        super(JournalEntry, self)._init_(*args, **kwargs)
    ...
    def update_gl_entry(self):
        for d in self.get("accounts"):
            frappe.db.sql(
                """UPDATE `tabGL Entry` SET posting_date = %s WHERE voucher_no = %s AND account = %s""",
                (d.posting_date, self.name, d.account),
            )
        ...
        if (
            ...
            if account_currency == previous_account_currency:
                if self.total_credit != doc.total_debit or self.total_debit != doc.total_credit:
                    frappe.throw(
                        _("Total Credit/ Debit Amount should be same as linked Journal Entry")
                    )
    ...
    def update_asset_value(self):
        ...
        for d in self.get("accounts"):
            if (
                ...
                and d.reference_name not in processed_assets
            ):
                ...
                asset.db_set(
                    "value_after_depreciation", asset.value_after_depreciation - depr_value
                )
    ...
    def validate_cheque_info(self):
        ...
        if self.cheque_date and not self.cheque_no:
            msgprint(
                _("Reference No is mandatory if you entered Reference Date"), raise_exception=1
            )
            ...
            if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
                ...
                if d.is_advance == "Yes":
                    if d.party_type == "Customer" and flt(d.debit) > 0:
                        frappe.throw(
                            _("Row {0}: Advance against Customer must be credit").format(d.idx)
                        )
                    elif d.party_type == "Supplier" and flt(d.credit) > 0:
                        frappe.throw(
                            _("Row {0}: Advance against Supplier must be debit").format(d.idx)
                        )
        ...
        for d in self.get("accounts"):
            if d.reference_type == "Journal Entry":
                ...
                if d.reference_name == self.name:
                    frappe.throw(
                        _("You can not enter current voucher in 'Against Journal Entry' column")
                    )
        ...
        for d in self.get("accounts"):
            ...
            if d.reference_type and d.reference_name and (d.reference_type in list(field_dict)):
                ...
                if not against_voucher:
                    frappe.throw(
                        _("Row {0}: Invalid reference {1}").format(d.idx, d.reference_name)
                    )
        ...
        for reference_name, total in self.reference_totals.items():
            ...
            if reference_type in ("Sales Order", "Purchase Order"):
                ...
                if order.docstatus != 1:
                    frappe.throw(
                        _("{0} {1} is not submitted").format(reference_type, reference_name)
                    )

                if flt(order.per_billed) >= 100:
                    frappe.throw(
                        _("{0} {1} is fully billed").format(reference_type, reference_name)
                    )
                ...
                if flt(voucher_total) < (flt(order.advance_paid) + total):
                    frappe.throw(
                        ...
                        ).format(reference_type, reference_name, formatted_voucher_total)
                    )
    ...
    def validate_multi_currency(self):
        ...
        if alternate_currency:
            if not self.multi_currency:
                frappe.throw(
                    _("Please check Multi Currency option to allow accounts with other currency")
                )
    ...
    def set_amounts_in_company_currency(self):
        if not (self.voucher_type == "Exchange Gain Or Loss" and self.multi_currency):
            for d in self.get("accounts"):
                ...
                d.debit = flt(
                    d.debit_in_account_currency * flt(d.exchange_rate), d.precision("debit")
                )
                d.credit = flt(
                    d.credit_in_account_currency * flt(d.exchange_rate), d.precision("credit")
                )
    ...
    def make_gl_entries(self, cancel=0, adv_adj=0):
        ...
        if gl_map:
            make_gl_entries(
                gl_map, cancel=cancel, adv_adj=adv_adj, update_outstanding=update_outstanding
            )
    ...
    def set_account_and_party_balance(self):
        ...
        for d in self.get("accounts"):
            if d.account not in account_balance:
                account_balance[d.account] = get_balance_on(account=d.account, date=d.posting_date)
    ...
    party_row = je.append(
        "accounts",
        {
            ...
            "party_balance": get_balance_on(
                party=args.get("party"), party_type=args.get("party_type")
            ),
